...
period = 5
Natoms = 30 # change this to have more or fewer atoms
...
def setmass(evt): #установление массы
    global mass1
    mass1 = mass * (massslider.GetValue() + 250)/ 500
...
exit_program = wx.Button(p, label='ВЫХОД', pos=(win_width - 135,win_height-50), size = (135,50))
exit_program.Bind(wx.EVT_BUTTON, leave)

start = 0

def press_start(evt): #реакция на нажатие кнопки "старт"
    global start
    start = 1
    
start_button = wx.Button(p, label='СТАРТ', pos=(win_width - 270,win_height-50), size = (135,50))
start_button.Bind(wx.EVT_BUTTON, press_start)
...
def speed(time):
    if (time % 100 < 50):
        return sqrt(3 * 4E-3 / 6E23 * k * 300) / (5 * 4E-3 / 6E23)
    else:
        return -sqrt(3 * 4E-3 / 6E23 * k * 300) / (5 * 4E-3 / 6E23)
...
while(still_working == 0):
    ...
    for i in range(Natoms):
        ...
        v_sum += sqrt(mag2(p[i]))/mass
    graph_temp.plot(pos = (time, total_momentum / (3 * k * mass) / Natoms))
    #graph_average_speed.plot(pos = (time, v_sum / Natoms))
    #cредняя скорость
